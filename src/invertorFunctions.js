const divideIntoStrings = (
  ...
  export const getSuitableInvertorAndStrings = (appState, materialState) => {
    const invertors = require("./invertors.json");
    let panelCount = 0;
  
    const panelPower = materialState.panelPower;
    const panelVoltage = materialState.panelVoltage;
    const panelCurrent = materialState.panelCurrent;
  
    for (let panelsInRow of appState.layout.panels) {
      panelCount += panelsInRow;
    }
  
    const plantPower = panelCount * panelPower;
    const allowPowerReserve = appState.allowPowerReserve;
